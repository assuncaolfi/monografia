...
Nesta seção, fazemos uma breve revisão dos conceitos necessários para o 
desenvolvimento deste trabalho. Descrevemos conjuntos de estados e 
exemplificamos alguns deles, modelos epidemiológicos e também modelos de 
grafos aleatórios.
...
estado ao longo do tempo \cite{infectiousdiseases}. Existem três maneiras 
de definir esse tipo de modelo: determinística e estocasticamente, e em redes. 
Nesta seção, detalharemos modelos determinísticos, comentaremos modelos 
estocásticos e faremos sua conexão com o tipo de modelo que será usado ao longo 
deste trabalho: modelos em redes.
...
idênticas de se encontrarem), o $SIR$ determinísticos é definido por um 
sistema de equações diferenciais \cite{keeling2005}. Seja $t$ o tempo, o número 
de Susceptíveis $N_S(t)$, Infectados $N_I(t)$ e Recuperados $N_R(t)$ na 
população:

$$
\begin{aligned} 
\frac{dN_S(t)}{dt} &= -\frac{\beta N_I(t)N_S(t)}{N}, \\ 
\frac{dN_I(t)}{dt} &=\frac{\beta N_I(t)S(t)}{N} -\gamma N_I(t),\\
\frac{dN_R(t)}{dt} &=\gamma N_I(t).
\end{aligned}
$$
onde $\beta$ é a taxa de contatos entre suscetíveis e infectados, e a 
probabilidade de ser infectado é $\frac{ N_I(t)}{N}$. A taxa de recuperação é
$\gamma$. Se $N$ é o tamanho constante da população, então 
$N_S(t)+N_I(t)+N_R(t)=N$.

Dados os mesmos pressupostos, o $SEIQHRF$ é definido por um conjunto similar. 
Exemplificamos o sistema abaixo, mas não detalharemos o significado de cada 
parâmetro. Esses significados podem ser inferidos pela Figura 
\@ref(fig:seiqhrf-plot) e são tratados empiricamente na seção de simulações
(ver Tabelas \@ref(tab:tab-inf-pars) e \@ref(tab:tab-prog-pars)).
...
Existe também uma definição estocástica para esse tipo de modelo, que é similar 
à definição de redes que daremos na próxima subseção, mas sem estruturas de 
vizinhança. Como a definição é bem similar à de redes, não a detalhamos neste 
trabalho.
...
Existe um caso especial de TERGMs, que são os _Separable TERGMs_. São 
especificações nas quais os parâmetros de formação e dissolução de arestas 
são independentes uns dos outros, por isso o nome _separável_. No contexto 
deste trabalho, todo TERGM é um STERGM.
...
Número de arestas - tamanho do conjunto $\{(i, j) \vert Y_{ij} = 1\}$.

`nodematch(attr)`: 

Homofilia - tamanho dos conjuntos 
$\{(i, j) | \vert Y_{ij} = 1, \text{atributo}_i = \text{atributo}_j\}$
para cada atributo.

`absdiff(attr)`

Diferença absoluta - 
$\sum_{(i,j) \vert Y_{ij} = 1}| \text{atributo}_i - \text{atributo}_j|$

`gwesp`

Número de _shared partners_ da rede. 
Tamanho do conjunto $\{(i,j)|Y_{ij} = 1, Y_{ik} = 1, Y_{jk} = 1\}$.
...
seguir. As funções completas em R estão disponíveis no Apêndice A.
...
\subsection{infect e progress}

A função `infect` é o módulo responsável por realizar o processo de infecção a 
cada tempo $t$ da simulação. Simplificadamente, a função pode ser descripta 
pelo pseucódigo:

\begin{algorithm}[H]
\DontPrintSemicolon
\SetAlgoLined
\KwResult{Simulação no tempo t}
\SetKwInOut{Input}{Input}\SetKwInOut{Output}{Output}
\Input{
    Grafo no tempo t,
    Estados dos vértices no tempo t,
    Parâmetros de infecção
  }
\Output{
    Estados dos vértices após a infecção no tempo t
  }
\BlankLine
arestas discordantes = arestas que conectam uma pessoa suscetível a outra em
algum estado infeccioso\;
\For{todas as as arestas discordantes}{
    o vértices suscetível é infectado com probabilidade = probabilidade dos indíduos se encontrarem e se infectarem\;
}
\caption{infect}
\end{algorithm} 
...
Analogamente, há também a função `progress`, que é responsável pela progressão 
entre estados do modelo. Ela está simplificada no pseudocódigo:
...
\Output{
    Estados dos vértices após a progressão no tempo t
  }
\BlankLine
vértices com possibilidade de progressão = vértices que estão em algum estado 
que pode progredir a outro estado\;
\For{todas as arestas com possibilidade de progressão}{
    o vértice progride com probabilidade = 1 - probabilidade de não progredir a 
    estado algum\;
    o vértice progride para o estado com probabilidade proporcional às 
    taxas de progressão do estado em questão\;
}
\caption{progress}
\end{algorithm} 
...
A documentação do `ergm` contém detalhes sobre a coleta dos dados do grafo 
Magnolia original, e também sobre o modelo utilizado para anonimizar os 
dados e simular o `faux.magnolia.high`. Detalhes sobre o modelo estão na 
próxima subseção, e detalhes sobre a coleta na citação direta abaixo:  
...
```{r plot-sim-A, fig.cap = "Prevalência ao longo do tempo no Cenário B. Linhas são prevalências médias, e áreas são intervalos de prevalência."}
plot(
  sim_A, 
  popfrac = TRUE,
  xlab = "Tempo",
...
Existem outras análises interessantes que podem ser feitas a partir do objeto 
final da simulação. Por exemplo, a prevalência agregada por turma ou raça. 
Contudo, não exploraremos essas análises neste trabalho.
...
A implementação do modelo foi bem sucedida, bem como a organização das 
referências necessárias para entendê-lo. O modelo está disponível no Github 
para futuras pesquisas e extensões.

O objetivo da monografia foi demonstrar o funcionamento e o contexto do 
$SEIQHRF$ em redes, e um próximo passo é simular esse modelo nas redes 
amostradas do Aglomerado da Serra, no projeto *COVID-19: proposta de um modelo
epidemiológico que incorpora estruturas sociais de contágio*.
