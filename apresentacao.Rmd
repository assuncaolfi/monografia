...
$Y_{ij}$ denota o valor das arestas conectando os vértices $(i,j)$.
...
* `edges`: número de arestas - tamanho do conjunto $\{(i, j) \vert Y_{ij} = 1\}$
* `nodematch(attr)`: homofilia - tamanho dos conjuntos 
...
* `gwesp`: número de _shared partners_ da rede. 
...
plot_attribute_distribution <- function(attribute) {
  ...
  barplot(prop.table(tab), names.arg = names(tab), main = attribute, col = COLORS[3], density = 50)
}
